      ...
      <a-form :labelCol="{ span: 6, offset: 0 }" :wrapper-col="{span:18}">
        <a-row :gutter="16" justify="start">
          <a-col :xs="24" :sm="24" :md="24" :lg="6" :xl="6">
            <a-form-item label="部门名称：" v-bind="searchValidateInfos.deptId">
              <a-input placeholder="请输入" v-model:value="searchModelRef.deptId"/>
            </a-form-item>
          </a-col>
          <a-col :xs="24" :sm="24" :md="24" :lg="6" :xl="6">
            <a-form-item label="用户名：" v-bind="searchValidateInfos.username">
              <a-input placeholder="请输入" v-model:value="searchModelRef.username"/>
            </a-form-item>
          </a-col>
          <a-col :xs="24" :sm="24" :md="24" :lg="6" :xl="6">
            <a-form-item label="昵称：" v-bind="searchValidateInfos.nickName">
              <a-input placeholder="请输入" v-model:value="searchModelRef.nickName"/>
            </a-form-item>
          </a-col>
          <a-col :xs="24" :sm="24" :md="24" :lg="6" :xl="6">
            <a-form-item label="手机号码：" v-bind="searchValidateInfos.phone">
              <a-input placeholder="请输入" v-model:value="searchModelRef.phone"/>
            </a-form-item>
          </a-col>
          <a-col :xs="24" :sm="24" :md="24" :lg="6" :xl="6">
            <a-form-item label="是否为admin账号：" v-bind="searchValidateInfos.isAdmin">
              <a-radio-group v-model:value="searchModelRef.isAdmin">
          ...
          <a-col :xs="24" :sm="24" :md="24" :lg="6" :xl="6">
            <a-form-item label="状态：1启用、0禁用：" v-bind="searchValidateInfos.enabled">
          ...
          <a-col :xs="24" :sm="24" :md="24" :lg="6" :xl="6">
            <a-form-item label="账号到期时间：" v-bind="searchValidateInfos.expireTime">
              <a-date-picker v-model:value="searchModelRef.expireTime" valueFormat='yyyy-MM-DD HH:mm:ss'/>
            </a-form-item>
          </a-col>
          <a-col :xs="24" :sm="24" :md="24" :lg="6" :xl="6">
            <a-form-item label="账号是否未锁定：" v-bind="searchValidateInfos.nonLocked">
              <a-radio-group v-model:value="searchModelRef.nonLocked">
                <a-radio :style="radioStyle" :value="null">全部</a-radio>
                <a-radio :style="radioStyle" :value="true">是</a-radio>
                <a-radio :style="radioStyle" :value="false">否</a-radio>
              </a-radio-group>
        ...
        <template #action="{ record }">
          <a-row type="flex" justify="center">
            <a-col flex="1" offset=2>
              <a-button type="primary" size="small" @click.stop="() => detailUpdateData(record.userId)"
                        :loading="detailUpdateLoading.includes(record.userId)">编辑
              </a-button>
            </a-col>
            <a-col flex="1" offset=2>
              <a-button type="danger" size="small" @click.stop="() => deleteTableData(record.userId)"
                        :loading="deleteLoading.includes(record.userId)">删除
              </a-button>
            </a-col>
          </a-row>
        </template>
...
import {PaginationConfig, SysUserDataType, TableListQueryParams} from './data.d';
...
interface ListSearchTablePageSetupData {
  ...
  createSubmit: (values: Omit<SysUserDataType, 'userId'>, resetFields: (newValues?: Props | undefined) => void) => Promise<void>;
  detailUpdateLoading: number[];
  detailUpdateData: (userId: number) => Promise<void>;
  updateData: Partial<SysUserDataType>;
  updateFormVisible: boolean;
  updateFormCancel: () => void;
  updateSubmitLoading: boolean;
  updateSubmit: (values: SysUserDataType, resetFields: (newValues?: Props | undefined) => void) => Promise<void>;
  deleteLoading: number[];
  deleteTableData: (userId: number) => void;
  searchOpen: boolean;
  setSearchOpen: () => void;
  ...
  rowClick: (record: SysUserDataType, index: number) => {};
  rowSelection: {};
...
export default defineComponent({
  name: 'SysUserListSearchTablePage',
  components: {
    CreateForm,
    UpdateForm,
    DownOutlined,
  ...
  setup(): ListSearchTablePageSetupData {
    ...
    const rowClick = (record: SysUserDataType, index: number) => {
      return {
        // 点击行
        onClick: () => {
          console.log(record);
    ...
    const rowSelection = {
      onChange: (selectedRowKeys: (string | number)[], selectedRows: SysUserDataType[]) => {
        // console.log('onChange');
        // console.log(`selectedRowKeys: ${selectedRowKeys}`, 'selectedRows: ', selectedRows);

      },
      onSelect: (record: SysUserDataType, selected: boolean, selectedRows: SysUserDataType[]) => {
      ...
      onSelectAll: (selected: boolean, selectedRows: SysUserDataType[], changeRows: SysUserDataType[]) => {
    ...
    const store = useStore<{ SysUserListSearchTable: ListStateType }>();

    // 列表数据
    const list = computed<SysUserDataType[]>(() => store.state.SysUserListSearchTable.tableData.list);

    // 列表分页
    const pagination = computed<PaginationConfig>(() => store.state.SysUserListSearchTable.tableData.pagination);
    ...
    const getList = async (current: number, pageSize: number): Promise<void> => {
      ...
      try {
        ...
        await store.dispatch('SysUserListSearchTable/queryTableData', {
          ...fieldsValue,
          ...{
            page: current,
            size: pageSize,
    ...
    // 新增弹框 - 提交
    const createSubmit = async (values: Omit<SysUserDataType, 'userId'>, resetFields: (newValues?: Props | undefined) => void) => {
      createSubmitLoading.value = true;
      const res: boolean = await store.dispatch('SysUserListSearchTable/createTableData', values);
      if (res) {
        resetFields();
    ...
    const updateFormCancel = () => {
      ...
      store.commit('SysUserListSearchTable/setUpdateData', {});
    }
    // 编辑弹框 - 提交 loading
    const updateSubmitLoading = ref<boolean>(false);
    // 编辑弹框 - 提交
    const updateSubmit = async (values: SysUserDataType, resetFields: (newValues?: Props | undefined) => void) => {
      updateSubmitLoading.value = true;
      console.log("values:", values);
      console.log("newValues:", values);
      const res: boolean = await store.dispatch('SysUserListSearchTable/updateTableData', values);
      if (res) {
        updateFormCancel();
    ...
    // 编辑弹框 data
    const updateData = computed<Partial<SysUserDataType>>(() => store.state.SysUserListSearchTable.updateData);
    const detailUpdateLoading = ref<(string | number)[]>([]);
    const detailUpdateData = async (userId: number) => {
      detailUpdateLoading.value = [userId];
      const res: boolean = await store.dispatch('SysUserListSearchTable/queryUpdateData', userId);
      if (res) {
    ...
    // 删除
    const deleteTableData = (userId: number) => {
      Modal.confirm({
        title: '删除',
        content: '确定删除吗？',
        okText: '确认',
        ...
        onOk: async () => {
          ...
          const res: boolean = await store.dispatch('SysUserListSearchTable/deleteTableData', userId);
          if (res) {
    ...
    const batchDeleteTableData = async () => {
      ...
      Modal.confirm({
        ...
        onOk: async () => {
          ...
          const res: boolean = await store.dispatch('SysUserListSearchTable/batchDeleteTableData', selectIds.value);
          if (res) {
            message.success('删除成功！');
            await getList(pagination.value.current, pagination.value.pageSize);
          }
    ...
    return {
      columns,
      list: list as unknown as SysUserDataType[],
      ...
      detailUpdateData,
      updateData: updateData as Partial<SysUserDataType>,
