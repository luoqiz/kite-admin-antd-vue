...
import { SysUserDataType, TableDataType, TableListQueryParams } from './data.d';
import {
    queryList,
    removeData,
    createData,
...
const StoreModel: ModuleType = {
    ...
    async createTableData({ commit }, payload: Pick<SysUserDataType,  "userId" | "password"> ) {
        try {
    ...
    async updateTableData({ commit }, payload: SysUserDataType ) {
            try {
                const { userId } = payload;
                await updateData(userId, payload);
                return true;
            } catch (error) {
                return false;
            }
