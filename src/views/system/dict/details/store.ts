...
import { SysDictDetailDataType, TableDataType, TableListQueryParams } from './data.d';
import {
    queryList,
    removeData,
    createData,
...
const StoreModel: ModuleType = {
    namespaced: true,
    name: 'SysDictDetailListSearchTable',
    state: {
        ...initState
    },
    mutations: {
    ...
    async createTableData({ commit }, payload: Pick<SysDictDetailDataType,  "detailId" |  "label" |  "value"> ) {
        try {
    ...
    async updateTableData({ commit }, payload: SysDictDetailDataType ) {
            try {
                const { detailId } = payload;
                await updateData(detailId, payload);
                return true;
            } catch (error) {
                return false;
            }
