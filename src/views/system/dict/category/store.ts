...
import { SysDictDataType, TableDataType, TableListQueryParams } from './data';
import {
    queryList,
    removeData,
    createData,
...
const StoreModel: ModuleType = {
    ...
    async createTableData({ commit }, payload: Pick<SysDictDataType,  "dictId" |  "name"> ) {
        try {
    ...
    async updateTableData({ commit }, payload: SysDictDataType ) {
            try {
                const { dictId } = payload;
                await updateData(dictId, payload);
                return true;
            } catch (error) {
                return false;
            }
