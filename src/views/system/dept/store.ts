...
import { SysDeptDataType, TableDataType, TableListQueryParams } from './data.d';
import {
  batchRemoveData,
  createData,
  detailData,
...
export interface ModuleType extends StoreModuleType<StateType> {
  ...
  actions: {
    queryAllTableData: Action<StateType, StateType>;
...
const StoreModel: ModuleType = {
  ...
  actions: {
    async queryAllTableData({ commit }, payload: TableListQueryParams) {
      try {
    ...
    async createTableData(
      { commit },
      payload: Pick<SysDeptDataType, 'deptId' | 'name' | 'enabled'>
    ) {
      try {
    ...
    async updateTableData({ commit }, payload: SysDeptDataType) {
      try {
        const { deptId } = payload;
        await updateData(deptId, payload);
        return true;
      } catch (error) {
        return false;
      }
