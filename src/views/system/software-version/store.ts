...
import { SysSoftwareVersionDataType, TableDataType, TableListQueryParams } from './data.d';
import {
    queryList,
    removeData,
    createData,
...
const StoreModel: ModuleType = {
    namespaced: true,
    name: 'SysSoftwareVersionListSearchTable',
    state: {
        ...initState
    },
    mutations: {
    ...
    async createTableData({ commit }, payload: Pick<SysSoftwareVersionDataType,  "id" |  "type"> ) {
        try {
    ...
    async updateTableData({ commit }, payload: SysSoftwareVersionDataType ) {
            try {
                const { id } = payload;
