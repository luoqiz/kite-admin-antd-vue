...
import { SysJobDataType, TableDataType, TableListQueryParams } from './data.d';
import {
    queryList,
    removeData,
    createData,
...
const StoreModel: ModuleType = {
    ...
    async createTableData({ commit }, payload: Pick<SysJobDataType,  "jobId" |  "name" |  "enabled"> ) {
        try {
    ...
    async updateTableData({ commit }, payload: SysJobDataType ) {
            try {
                const { jobId } = payload;
                await updateData(jobId, payload);
                return true;
            } catch (error) {
                return false;
            }
